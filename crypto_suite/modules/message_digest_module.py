...
MÓDULO: Message Digest (Punto 1a)

Implementación completa de generación de resúmenes digitales
...
class MessageDigestModule:
    """Módulo de Message Digest con interfaz gráfica"""
    ...
    def generate_digest(self):
        ...
        output = f"Algoritmo: {algorithm.upper()}\n"
        output += f"Tamaño: {hash_obj.digest_size} bytes ({hash_obj.digest_size * 8} bits)\n"
        output += f"Bloque: {hash_obj.block_size} bytes\n\n{'='*60}\n\n"
        output += f"Digest (Hex):\n{digest}\n\n{'='*60}\n"
    ...
    def analyze_avalanche(self):
        ...
        self.visualize_avalanche(hash1, hash2, diff_bits, total_bits, percentage, message, modified)
    ...
    def compare_messages(self):
        ...
        if not msg1 or not msg2:
            messagebox.showwarning("Advertencia", "Ingrese ambos mensajes")
            return
        ...
        for algo in ['md5', 'sha1', 'sha256', 'sha512']:
            hash1 = hashlib.new(algo, msg1.encode()).hexdigest()
            hash2 = hashlib.new(algo, msg2.encode()).hexdigest()
            match = hash1 == hash2
            
            output += f"{algo.upper()}:\n"
            output += f"  Msg 1: {hash1}\n"
            output += f"  Msg 2: {hash2}\n"
            output += f"  Match: {'✓ SÍ' if match else '✗ NO'}\n\n"
        
        output += "=" * 60 + "\n"
        output += f"RESULTADO: {'IDÉNTICOS ✓' if msg1 == msg2 else 'DIFERENTES ✗'}"
